...
4. Iterate the plan's waves in topological order. For the first task whose `state.tasks[task.id].status` is not `'passed'`, invoke `anvil run --task <id>`. The dispatcher identifier defaults to `anvil_subagent`; `state.run_id` is preserved.
